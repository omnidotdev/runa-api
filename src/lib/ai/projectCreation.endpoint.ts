...
const projectCreationRoutes = new Elysia({
  ...
  .post(
    ...
    async ({ body, auth, set }) => {
      ...
      const orgAccessCheck = await checkOrganizationAccess(
        body.organizationId,
        auth.organizations,
      );
      if (!orgAccessCheck.ok) {
        set.status = orgAccessCheck.status;
        return orgAccessCheck.response;
      }

      const { organizationId, organizationSlug } = orgAccessCheck.value;
      ...
      if (body.sessionId) {
        let loadedSession: SelectAgentSession | null =
            ...
            try {
              ...
              if (proposal.initialTasks && proposal.initialTasks.length > 0) {
                const taskCountResult = await dbPool
                  ...
                  .where(
                    eq(projects.organizationId, toolContext.organizationId),
                  );
