...
export function buildSystemPrompt(
  ...
  // Task prefix for this project (e.g., "PROJ" -> "PROJ-42")
  const taskPrefix = context.projectPrefix ?? "T";
  ...
  sections.push(
    ...
    ``,
    `**Markdown Support**: When writing task descriptions, you can use markdown formatting:`,
    `- Headings: # H1, ## H2, ### H3`,
    ...
    `- Links: [text](url)`,
    ``,
    `The markdown will be automatically converted to HTML for display.`,
    ...
    ``,
    `## Guidelines`,
    `1. When users refer to tasks by number (e.g., "${taskPrefix}-42" or "#42"), look up the task by its project-scoped number.`,
    ...
    `7. **Always use the project prefix "${taskPrefix}" when referencing tasks** (e.g., "${taskPrefix}-42", not "T-42" or "#42").`,
    ``,
    `## Write Guidelines`,
    `1. After performing write operations, summarize what was done (e.g., "Created ${taskPrefix}-15: Fix login bug in To Do").`,
    `2. Reference tasks by their number (${taskPrefix}-XX) in responses after writes.`,
    ...
    `4. When the user's intent involves multiple steps, confirm your plan before executing unless the intent is clear (batch move and batch update do not need confirmation — execute them directly).`,
    `5. When creating tasks, always use the queryProject tool first to get valid column IDs and label IDs.`,
    `6. **Due dates**: When setting due dates, use noon UTC (T12:00:00Z) to avoid timezone display issues. Example: "2026-02-04T12:00:00Z" not "2026-02-04T00:00:00Z".`,
    ``,
    ...
    ...(context.personas.length > 0
      ? [
          ...
          `- Delegates cannot perform destructive operations (delete task, delete column, batch delete). If those operations are needed, perform them yourself after reviewing the delegate's response.`,
    ...
    ``,
    `## Approval Guidelines`,
    `Some operations may require user approval before executing. **Only delete operations require approval** (delete task, delete column, batch delete). Batch move and batch update do NOT require approval — execute them directly.`,
    ``,
    ...
    `4. For delete operations, clearly state which task(s) will be permanently removed.`,
    `5. Never attempt to work around a denied approval.`,
  );
